...
mdb.prototype._open = function(callback) {
	...
	if( !self.isopen ) {
		...
		var client = new mongodb.Db(self.dbname, new mongodb.Server(self.options.host, self.options.port, self.options), self.options);
			...
			if(!err && self.options.username){
				client.authenticate(self.options.username, self.options.password, function(err, success){
					if(success){
						...
						handle(err ? err : new Error('Could not authenticate user ' + self.options.username), client);
					}
				});
			}
			else {
